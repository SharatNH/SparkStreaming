...
object StructuredStreaming {
   ...
   def parseDateField(field: String): Option[String] = {
     ...
     // taking data and converting it into sql timestamp
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        //getorcreate--if we have saved at some checkup pickup from there
        //if new run then we create again
      setupLogging()
      ...
      // (instead of "append").
      //outputMode -- complelte - if we were aggregating info it overtime
      // append if not
