...
object LogAlarmer {
  ...
  def main(args: Array[String]) {
    ...
    statusCounts.foreachRDD((rdd, time) => {
      ...
      if (rdd.count() > 0) {
        //collect will transform the rdd into array
        //basic idea is to process individual rdd and get the count
...
//issues alarm only once in every half an hour, so will issue next alarm after another 30 min
//extract parameters like window length, host and port to connect to and max ratio as command line arguments
// extend alarm if no data comes for certain duration
