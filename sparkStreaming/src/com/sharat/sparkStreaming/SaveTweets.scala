...
object SaveTweets {
  ...
  def main(args: Array[String]) {
    ...
    // process one bit at a time as it comes by...
    //doesnt wait for Dstream to terminate and later go back and start
    statuses.foreachRDD((rdd, time) => {
      // Don't bother with empty batches
      if (rdd.count() > 0) {
        // Combine each partition's results into a single RDD:
        // consolidate the rdd into single partition...since it might be 
        //distributed. its imp if we want to dump it to a db, since that connection
        //may not be valid in all the pcs. so one connection to one collection
        
        //cache is called since if two or more actions on rdd is called, spark will again build DAG
        //and reprocess the RDD from groundup
